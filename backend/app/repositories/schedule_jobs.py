...
def create_item(db: Session, name: str, schedule_type: str, cron_value=None, interval_minutes=None, approved_query_id=None, message_template_id=None, notify_room_id=None, use_alertroom='N', next_run_at=None, is_active='Y', payload=None, payload_json=None):
    row = ScheduleJob(name=name, schedule_type=schedule_type, cron_value=cron_value, interval_minutes=interval_minutes, approved_query_id=approved_query_id, message_template_id=message_template_id, notify_room_id=notify_room_id, use_alertroom=use_alertroom or 'N', next_run_at=next_run_at, is_active=is_active, payload_json=payload_json if payload_json is not None else payload)
    db.add(row)
    db.commit()
    db.refresh(row)
    return row
