...
def build_full_table_flex(rows:list[dict], title:str="จำนวนผู้ป่วยนัดแยกรายคลินิก", chunk_size:int=8, return_list:bool=False):
    if not rows:
        rows = [{}]
    ...
    for chunk, offset in _chunk(rows, chunk_size):
        ...
        for idx, row in enumerate(chunk, start=offset+1):
            clinic = row.get("clinic_name") or "-"
            dept = row.get("department") or "-"
            total = row.get("total_appointment") or 0
            contents.extend([
                {
                    "type":"box","layout":"baseline","margin":"md",
                    "contents":[
